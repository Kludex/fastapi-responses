...
def raise_another():
    raise HTTPException(status_code=status.HTTP_200_OK, detail="Another function!")


async def get_another_user():
    raise HTTPException(status_code=status.HTTP_304_NOT_MODIFIED, detail="Yet another function!")


def get_user(opa: str = Depends(get_another_user)):
    raise HTTPException(status_code=status.HTTP_201_CREATED, detail="HAHA")
